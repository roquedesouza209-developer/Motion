...
import type { ChatAttachmentType, PostKind } from "@/lib/server/types";
...
function classifyChatAttachmentType(mimeType: string): ChatAttachmentType | null {
  if (IMAGE_TYPES.has(mimeType)) {
    return "image";
  }
...
export async function storeUploadedChatMedia({
  file,
}: {
  file: File;
}): Promise<{ mediaUrl: string; mediaType: ChatAttachmentType; mimeType: string; name: string }> {
  if (!file || file.size <= 0) {
    throw new Error("Upload a non-empty media file.");
  }
  ...
  if (mediaType === "audio" && file.size > MAX_AUDIO_BYTES) {
    throw new Error("Voice messages must be 20MB or smaller.");
  }
